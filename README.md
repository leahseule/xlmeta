...
[**▶ 라이브 데모 열기**](https://xlmeta.weaveapp.duckdns.org) &nbsp;·&nbsp; [빠른 시작](#빠른-시작) &nbsp;·&nbsp; [무엇을 뽑나](#무엇을-뽑나) &nbsp;·&nbsp; [승인 레이어](#승인-레이어) &nbsp;·&nbsp; [설계](#설계)
...
| **승인 레이어** | 정의의 *내용*에 승인을 붙여, 의미가 바뀌면 자동 만료 |
...
*"이 정의는 담당자가 확인함"* — 추출만으로는 부족합니다. **틀린 정의가 승인된 채로 AI에게 가는 것**이
최악이니까요. 그래서 승인을 **이름이나 셀 주소가 아니라 정의의 '내용'에 붙입니다.**

```bash
python -m xlmeta report.xlsx --approve 발생원가 --by 홍길동   # 담당자가 승인
```

정의를 **정규화**해 지문(fingerprint)을 만듭니다. 셀 주소·시트 이름·행 개수는 지우고 **구조와 조건만** 남깁니다:
...
| 조건 `"승인"` → `"완료"` | **달라짐** | ⏳ 자동 만료 |
| 분모에 예비비 추가 | **달라짐** | ⏳ 자동 만료 |

**변화 감지와 승인 만료가 같은 한 장치입니다.** 사람이 "어? 이거 바뀌었나?" 하고 알아챌 필요가
없습니다. 기본값이 대기(pending)라, 변화를 놓쳐도 틀린 정의가 AI에게 가지 않습니다.

## AI에게 넘기기

엑셀을 올리면 구조·업무규칙을 **결정론적으로 요약**해 공개 페이지(`/s/<id>`)로 만들고, 그 링크를
**ChatGPT·Claude 프리필**에 담아 넘깁니다. 요약이 URL이 아니라 링크가 가리키는 페이지에 있어
주소가 길어지지 않고, **원시 데이터 값은 담지 않아**(구조·규칙만) 링크로 외부에 나가도 안전합니다.
